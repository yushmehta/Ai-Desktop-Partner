...
def send_message(chat_id):
    try:
        ...
        # If this is not a regeneration, add user message to chat
        ...
        # Log the usage as regenerate or message type
        usage_type = 'regenerate' if is_regenerate else 'message'
        log_api_usage(api_key, model, usage_type, estimate_tokens(user_message, attachments), attachments)
        
        # Handle response based on whether this is a regeneration or new message
        if is_regenerate and message_id:
            # Find the message to update
            message_updated = False
            for i, msg in enumerate(chat_data['messages']):
                if msg.get('id') == message_id:
                    # If the message doesn't have alternative_responses field, create it
                    if 'alternative_responses' not in msg:
                        msg['alternative_responses'] = [msg['content']]
                    
                    # Add the new response to alternatives
                    msg['alternative_responses'].append(response)
                    
                    # Update the current message content with the new response
                    msg['content'] = response
                    
                    # Create a copy of the message for the response (don't modify the original in the array)
                    ai_message_obj = {
                        'id': msg.get('id'),
                        'role': 'assistant',
                        'content': response,
                        'timestamp': datetime.now().isoformat(),
                        'alternative_responses': msg['alternative_responses'],
                    }
            ...
            if not message_updated:
                # If messageId not found, treat as new message
                ai_message_obj = {
                    'id': str(uuid.uuid4()),
                    'role': 'assistant',
                    'content': response,
                    'timestamp': datetime.now().isoformat(),
                    'alternative_responses': [response]
                }
            ...
            ai_message_obj = {
                ...
                'content': response,
                'timestamp': datetime.now().isoformat(),
                'alternative_responses': [response]  # Initialize with the first response
            }
            
            chat_data['messages'].append(ai_message_obj)
        
        # Update chat name based on conversation if it's still "New Chat" and we have at least 2 exchanges
        if chat_data['name'] == 'New Chat' and len(chat_data['messages']) >= 4 and not is_regenerate:  # 2 user messages + 2 AI responses
            try:
